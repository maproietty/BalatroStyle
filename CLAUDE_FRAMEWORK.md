...
- **HUD pipeline** — `ScoreHUD` is a pure listener: subscribes to `ScoreManager.OnScoreChanged` (chips×mult label), `ScoreManager.OnScoreReset` (snap to zero on new game), `HandScorer.OnHandEvaluated` (authoritative source of the rolling delta), `HandActionController.OnHandPlayed`/`OnHandDiscarded`/`OnSelectionCountChanged`, and `GameManager.OnRoundStarted`. Holds a single `[SerializeField] ScoreManager` ref (same sanctioned pattern as GameManager/RoundManager) solely to read `TotalScore` during `RefreshAll`. `HandNameBanner` subscribes only to `HandScorer.OnHandEvaluated` — decoupled from scoring state.
- **Score event semantics** — `ScoreManager` is the source of truth; it fires each event exactly once per action. `OnScoreChanged(chips, mult)` fires on apply or reset. `OnScoreRolled(delta, magnitude)` fires once per `ApplyScore` (one-shot juice trigger — used by `CameraEffects` for bloom pulse + chromatic aberration). `OnScoreReset` fires when state is zeroed. `TotalScore` is assigned **before** `OnScoreChanged` fires so any synchronous read is consistent. The rolling-counter animation is owned solely by `ScoreHUD`, driven off `HandScorer.OnHandEvaluated.FinalScore`.
- **Screen shake channel** — There is a single authoritative source per action: `HandScorer` fires a hand-strength-scaled shake on play; `HandActionController` fires a smaller shake on discard. `ScreenShake` no longer subscribes to `ScoreManager.OnScoreRolled` (was redundant with the hand-strength shake and caused double-fire).
- **Card composition of layout + selection + hover** — `Card.basePosition` is the raw layout target set by `MoveToPosition`. `selectionOffset` is an additive vector updated on `TrySetSelected`. `AnimateHover` and `MoveToPosition` both compose `basePosition + selectionOffset` as the rest position, so a re-layout (e.g. post-play refill via `ArrangeCards`) preserves the selected lift. Hover coroutine is tracked in a field and stopped individually — `StopAllCoroutines` is no longer used on the card, so `SelectPop` / `BounceReject` / `RevealGlowPulse` run to completion regardless of hover transitions. Hover tilt is spring-driven on both Z (cursor X) and X (cursor Y) for a two-axis tilt-toward-cursor feel.
...
| 2026-04-18 | Review sweep: (1) collapsed duplicate rolling-score animation — `ScoreManager` no longer owns a display coroutine; `OnScoreRolled` now fires once per `ApplyScore`; `ScoreHUD` is the sole owner of the rolling counter driven off `HandScorer.OnHandEvaluated.FinalScore`. (2) Added `ScoreManager.OnScoreReset` (replaces magic-value reset detection in HUD). (3) Removed double screen-shake on play — `HandActionController` no longer fires a generic shake; `HandScorer` hand-strength shake is the sole source. Added discard shake. `ScreenShake` dropped redundant `OnScoreRolled` subscription. (4) Fixed `CameraEffects` BloomPulse coroutine stacking (side-effect of one-shot `OnScoreRolled`). (5) `Card`: replaced `StopAllCoroutines` with a tracked `hoverRoutine` so `SelectPop` / `BounceReject` / `RevealGlowPulse` survive hover transitions. (6) `Card`: separated `selectionOffset` from `basePosition` — selection lift now survives re-layouts (`ArrangeCards` → `MoveToPosition` no longer wipes it). (7) `Card`: added spring-driven X-axis hover tilt so cards lean toward the cursor on both axes. (8) DRY: added `EvaluatedHand.TotalChips` + `FinalScore`; updated all three consumers. Framework phase tracker synced to reality. | 3–5 |
